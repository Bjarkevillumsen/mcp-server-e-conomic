...
service availability. Trust boundaries exist at the public Cloudflare edge,
Cloudflare-to-loopback proxy, MCP HTTP parser, Entra validation, tool dispatch,
...
| Oversized/malformed request or abuse | JSON/content-type validation, 1 MiB default limit, request timeout, Cloudflare abuse/rate controls | Distributed valid-token abuse needs monitoring and possible edge rate limits |
...
| Supply-chain/release tampering | Pinned upstream, lockfile, audit, WinSW checksum, manifest hashes, tagged artifact | GitHub/account compromise requires independent artifact provenance controls |
...
Endpoint/client-device compromise, Entra tenant administration, Cloudflare
account administration, Windows base-image hardening, and the correctness of
